...
public class MessagesController(IMessageRepository messageRepository, IMemberRepository memberRepository) : BaseApiController
{
    ...
    public async Task<ActionResult<MessageDto>> CreateMessage(CreateMessageDto createMessageDto)
    {
        ...
        if (sender.Id == recipient.Id) return BadRequest("You cannot send messages to yourself");
    ...
    public async Task<ActionResult> DeleteMessage(string id)
    {
        ...
        if (message.SenderId != memberId && message.RecipientId != memberId) return BadRequest("You cannot delete this messages");
